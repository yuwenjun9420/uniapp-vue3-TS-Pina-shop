...
import { PostLoginApi,PostLoginSimpleApi } from '@/api/login';
...
const onGetPhoneNumber:UniHelper.ButtonOnGetphonenumber = async (event) => {
	...
	let res = await PostLoginApi(params)
	console.log("res::::",res)
}

const phoneNumber = '18503052639'
const simpleLogin = async () => {
	let res = await PostLoginSimpleApi(phoneNumber)
	const profile = useMemberStore()
	profile.setProfile(res.result)
	uni.showToast({
		icon: 'success',
		title: '登录成功'
	})
