<template>
  <scroll-view scroll-y class="viewport">
    ...
    <view class="goods">
      ...
      <!-- 操作面板 -->
      <view class="action">
        <view class="item arrow" @tap="onOpenSkuPanel(SkuMode.Both)">
    ...
    <view class="detail panel">
      ...
      <view class="content">
        ...
        <!-- 图片详情 -->
        <image v-for="item,index in goodsInfo?.details.pictures" :key="index"
        mode="widthFix"  
        :src="item"
        ></image>
      </view>
  ...
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    <view class="icons">
      ...
      <!-- #endif -->
      <navigator class="icons-button" url="/pages/cart/cart1">
        <text class="icon-cart"></text>购物车
      </navigator>
    </view>
    <view class="buttons">
      <view class="addcart" @tap="onOpenSkuPanel(SkuMode.Cart)"> 加入购物车 </view>
      <view class="buynow" @tap="onOpenSkuPanel(SkuMode.Buy)"> 立即购买 </view>
    </view>
  </view>
...
// 获取屏幕边界到安全区域距离
  ...
  const onOpenSkuPanel = (val: SkuMode) => {
  ...
  const selectAddress = computed(()=>{
    return addressStore.addressInfo?.fullLocation || '请选择收获地址'
  })
  ...
  const onBuyNow = (event: SkuPopupEvent) => {
    uni.navigateTo({
      url: `/pagesOrder/order/order?skuId=${event._id}&count=${event.buy_num}`
    })
    console.log(event,"onBuyNow:::")
  }
...
</style>@/components/vk-data-goods-sku-popup/vk-data-goods-sku-popup